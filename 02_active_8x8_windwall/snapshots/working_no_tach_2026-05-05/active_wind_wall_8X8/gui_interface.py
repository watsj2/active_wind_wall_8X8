...
from config import BASE_FREQUENCY, NUM_MOTORS, PWM_MIN, PWM_MAX, GRID_ROWS, GRID_COLS, UPDATE_RATE_HZ, WALL_MOTOR_GRID
...
class WindWallGUI(QMainWindow):
    ...
    def create_grid_panel(self):
        ...
        layout_note = QLabel("Q1 top-left, Q2 bottom-left, Q3 top-right, Q4 bottom-right")
        layout_note.setAlignment(Qt.AlignmentFlag.AlignCenter)
        layout_note.setStyleSheet("color: #666; font-style: italic;")
        layout.addWidget(layout_note)
    ...
    def create_control_panel(self):
        ...
        bench_box = QGroupBox("Motor Test")
        bench_layout = QVBoxLayout()

        bench_layout.addWidget(QLabel("Motor:"))
        ...
        bench_layout.addWidget(self.bench_motor_select)

        bench_layout.addWidget(QLabel("PWM (microseconds):"))
        ...
        bench_layout.addWidget(self.bench_pwm)

        bench_layout.addWidget(QLabel("Duration (seconds):"))
        self.bench_duration = QDoubleSpinBox()
        self.bench_duration.setRange(0.5, 60.0)
        self.bench_duration.setSingleStep(0.5)
        self.bench_duration.setValue(10.0)
        self.bench_duration.setDecimals(1)
        bench_layout.addWidget(self.bench_duration)

        self.bench_status_label = QLabel("Idle")
        self.bench_status_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        bench_layout.addWidget(self.bench_status_label)
        ...
        bench_layout.addLayout(bench_btn_layout)
    ...
    def start_bench_test(self):
        ...
        self.bench_test_thread = threading.Thread(
            ...
            args=(motor_id, pwm_us, duration_s, self.bench_stop_event),
            daemon=True,
        )
        self.bench_test_thread.start()

    def run_bench_test_thread(self, motor_id, pwm_us, duration_s, stop_event):
        ...
        try:
            use_mock = platform.system() == "Darwin"
            hardware = HardwareInterface(use_mock=use_mock)
