...
If keeping the existing 8-output-per-Pico architecture:

- Existing layer: 8 Picos/controllers for 64 motors.
- Added layer: 8 additional Picos/controllers for 64 motors.
- New total: 16 Picos/controllers.
- Each controller handles 8 motors.

Recommended physical controller grouping:

```text
Controller 01 -> P01-P04 front/back depending on final protocol
Controller 02 -> next 8 motor outputs
...
Controller 16 -> final 8 motor outputs
```

Before wiring, decide whether controllers are grouped by:

- Layer-first: controllers 1-8 front, controllers 9-16 back.
- Pair-first: each controller controls four coaxial pixel pairs.

Pair-first is cleaner physically for coaxial wiring, but layer-first may be
easier if reusing the existing 64-motor wiring as-is.
...
6. Test one added controller with no motors connected.
7. Test one added ESC/motor at idle.
8. Test one coaxial pair at low PWM.
9. Test one full row or one controller group.
